# Bootstrappable Danxome Core Builds

This directory contains the files necessary to perform bootstrappable Danxome
Core builds.
...
To build Danxome Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Danxome Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/danxome-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/danxome-core/guix.sigs).
