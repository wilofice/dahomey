...
This directory contains the files necessary to perform bootstrappable Dahomey
Core builds.
...
To build Dahomey Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Dahomey Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/dahomey-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/dahomey-core/guix.sigs).
