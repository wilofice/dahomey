This directory contains the source code for the Dahomey Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Dahomey UI.
...
- ClientModel is used by the main application `dahomeygui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a dahomey:... link from a browser).
...
* UI elements like DahomeyAmountField, which inherit from QWidget.
* `dahomeystrings.cpp`: automatically generated
* `dahomeyunits.(h/cpp)`: DEY / mDEY / etc. handling
* `callback.h`
...
**Note:** Do not change `local/dahomey_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `dahomey-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `dahomey-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `dahomey-qt`
9. You're all set! Start developing, building, and debugging the Dahomey Core GUI
