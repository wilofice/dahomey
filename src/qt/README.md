This directory contains the source code for the Danxome Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Danxome UI.
...
- ClientModel is used by the main application `danxomegui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a danxome:... link from a browser).
...
* UI elements like DanxomeAmountField, which inherit from QWidget.
* `danxomestrings.cpp`: automatically generated
* `danxomeunits.(h/cpp)`: DAN / mDAN / etc. handling
* `callback.h`
...
**Note:** Do not change `local/danxome_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `danxome-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `danxome-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `danxome-qt`
9. You're all set! Start developing, building, and debugging the Danxome Core GUI
