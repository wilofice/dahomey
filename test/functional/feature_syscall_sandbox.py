...
"""Test danxomed aborts if a disallowed syscall is used when compiled with the syscall sandbox."""

from test_framework.test_framework import DanxomeTestFramework, SkipTest


class SyscallSandboxTest(DanxomeTestFramework):
    def set_test_params(self):
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        if not self.is_syscall_sandbox_compiled():
            raise SkipTest("danxomed has not been built with syscall sandbox enabled.")
        if self.options.nosandbox:
            raise SkipTest("--nosandbox passed to test runner.")

    def run_test(self):
        disallowed_syscall_terminated_danxomed = False
        ...
        with self.nodes[0].assert_debug_log([expected_log_entry]):
            ...
            try:
                ...
                disallowed_syscall_terminated_danxomed = True
        assert disallowed_syscall_terminated_danxomed
        self.nodes = []
