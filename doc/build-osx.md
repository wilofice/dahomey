...
This guide describes how to build dahomeyd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Dahomey Core from source.
...
### 4. Clone Dahomey repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Dahomey Core repository to a directory.
...
It is not necessary to build wallet functionality to run `dahomeyd` or  `dahomey-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/dahomey/dahomey/blob/master/doc/descriptors.md), `sqlite` is required.
...
Dahomey Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/dahomey/dahomey/issues/7714).
...
You can deploy a `.dmg` containing the Dahomey Core application using `make deploy`.
...
Run the following in your terminal to compile Dahomey Core:
...
## Running Dahomey Core

Dahomey Core should now be available at `./src/dahomeyd`.
If you compiled support for the GUI, it should be available at `./src/qt/dahomey-qt`.

The first time you run `dahomeyd` or `dahomey-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Dahomey/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Dahomey"

touch "/Users/${USER}/Library/Application Support/Dahomey/dahomey.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Dahomey/dahomey.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Dahomey/debug.log
```

## Other commands:

```shell
./src/dahomeyd -daemon      # Starts the dahomey daemon.
./src/dahomey-cli --help    # Outputs a list of command-line options.
./src/dahomey-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/dahomey-qt -server # Starts the dahomey-qt server mode, allows dahomey-cli control
```
