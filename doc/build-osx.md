...
This guide describes how to build danxomed, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Danxome Core from source.
...
### 4. Clone Danxome repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Danxome Core repository to a directory.
...
It is not necessary to build wallet functionality to run `danxomed` or  `danxome-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/danxome/danxome/blob/master/doc/descriptors.md), `sqlite` is required.
...
Danxome Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/danxome/danxome/issues/7714).
...
You can deploy a `.dmg` containing the Danxome Core application using `make deploy`.
...
Run the following in your terminal to compile Danxome Core:
...
## Running Danxome Core

Danxome Core should now be available at `./src/danxomed`.
If you compiled support for the GUI, it should be available at `./src/qt/danxome-qt`.

The first time you run `danxomed` or `danxome-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Danxome/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Danxome"

touch "/Users/${USER}/Library/Application Support/Danxome/danxome.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Danxome/danxome.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Danxome/debug.log
```

## Other commands:

```shell
./src/danxomed -daemon      # Starts the danxome daemon.
./src/danxome-cli --help    # Outputs a list of command-line options.
./src/danxome-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/danxome-qt -server # Starts the danxome-qt server mode, allows danxome-cli control
```
