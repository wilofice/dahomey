Dahomey Core version 0.18.0 is now available from:

  <https://dahomeycore.org/bin/dahomey-core-0.18.0/>
...
`/Applications/Dahomey-Qt` (on Mac) or `dahomeyd`/`dahomey-qt` (on
Linux).
...
recommended to use Dahomey Core on unsupported systems.
...
10.10. Additionally, Dahomey Core does not yet change appearance when
...
`contrib/init/dahomeyd.service`, it has been changed to use
`/var/lib/dahomeyd` as the data directory instead of
`~dahomey/.dahomey`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/dahomeyd` will
exist has enough space (check using `df -h /var/lib/dahomeyd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/dahomey/dahomey/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://dahomeycore.org/en/contact/) the Dahomey Core
...
The systemd init file (`contrib/init/dahomeyd.service`) has been changed
to use `/var/lib/dahomeyd` as the data directory instead of
`~dahomey/.dahomey`. This change makes Dahomey Core more consistent with
...
`datadir` option in `/etc/dahomey/dahomey.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/dahomey/dahomey.conf`.


Documentation
-------------

- A new short [document](https://github.com/dahomey/dahomey/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/dahomey/dahomey/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/dahomey/dahomey/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/dahomey/dahomey/blob/master/doc/dahomey-conf.md)
  about the `dahomey.conf` file describes how to use it to configure
  Dahomey Core.

- A new document introduces Dahomey Core's BIP174 [Partially-Signed
  Dahomey Transactions
  (PSBT)](https://github.com/dahomey/dahomey/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/dahomey/dahomey/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Dahomey-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Dahomey Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/dahomey/dahomey/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/dahomey/dahomey/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Dahomey Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Dahomey Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/dahomey/dahomey/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/dahomey/dahomey/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart dahomeyd with the `-deprecatedrpc=generate`
...
- A new `dahomey-wallet` tool is now distributed alongside Dahomey
...
This section describes planned changes to Dahomey Core that may affect
other Dahomey software and services.

- Since version 0.16.0, Dahomey Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Dahomey Core 0.20 (expected about
  a year after 0.18), Dahomey Core will default to native segwit
  ...
  to bech32 addresses, and if the Dahomey Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Dahomey Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Dahomey Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Dahomey Core
...
- A sub-project of Dahomey Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Dahomey Core.  See their [project
  page](https://github.com/dahomey-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Dahomey Core's own implementation, although entropy
  gathered by Dahomey Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Dahomey Core a
...
- On macOS, Dahomey Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `dahomey_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libdahomey_server.a` (jnewbery)
...
- #13926 [Tools] dahomey-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like dahomeyd (MarcoFalke)
...
- #14521 qt, docs: Fix `dahomey-qt -version` output formatting (hebasto)
...
- #11625 Add DahomeyApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libdahomey_server` (laanwj)
...
- #15154 Configure: dahomey-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/dahomey-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Dahomey Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libdahomey_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in dahomey-util-test.py (isghe)
...
- #12775 Integration of property based testing into Dahomey Core (Christewart)
...
- #12246 Bugfix: Only run dahomey-tx tests when dahomey-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_dahomey` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_dahomey` (MarcoFalke)
...
- #14186 dahomey-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_dahomey.conf` message on startup if conf file exists (leishman)
...
- #12255 Update dahomey.service to conform to init.md (dongcarl)
...
- #14497 Add doc/dahomey-conf.md (hebasto)
...
- #15354 Add missing `dahomey-wallet` tool manpages (MarcoFalke)
...
- #15765 correct dahomeyconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support dahomey folder on OSX (JimmyMow)
