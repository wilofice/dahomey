...
Danxome Core version 0.20.0 is now available from:

  <https://danxomecore.org/bin/danxome-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `DANXOME_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Danxome Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Danxome Core version 0.18.0.
...
- Testing or debugging of implementations of the Danxome P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Danxome Core. Danxome Core logs debug messages
...
- OpenSSL is no longer used by Danxome Core.  (#17265)

- BIP70 support has been fully removed from Danxome Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/danxome/danxome/issues/16599),
  [PR #16702](https://github.com/danxome/danxome/pull/16702), and the
  `danxomed help` for more information.  This option is experimental and
...
- All custom settings configured when Danxome Core starts are now
...
- The "Start Danxome Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://danxomecore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Danxome Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Danxome Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/danxome-core/HWI) for
  signing.  Future versions of Danxome Core should support a GUI option
...
- Danxome Core's automatically-generated source code documentation is
  now available at https://doxygen.danxomecore.org.  (#17596)
...
- The `danxome-cli` utility used with the `-getinfo` parameter now
...
- The `danxome-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=2022` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `danxomestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_danxome-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBDANXOME_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_danxome` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBDANXOME_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for danxome-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate danxome schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest danxome.conf (sdaftuar)
...
- #18022 Fix appveyor `test_danxome` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (danxome-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone danxome-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Danxome Core files (hebasto)
...
- #17119 Fix broken danxome-cli examples (andrewtoth)
...
- #17503 Remove bitness from danxome-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.danxomecore.org (laanwj)
...
- #17648 Rename wallet-tool references to danxome-wallet (hel-o)
...
- #18341 Replace remaining literal DAN with `CURRENCY_UNIT` (domob1812)
...
- #16115 On danxomed startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from danxome-cli -getinfo (malevolent)
