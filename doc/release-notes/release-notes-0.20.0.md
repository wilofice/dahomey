...
Dahomey Core version 0.20.0 is now available from:

  <https://dahomeycore.org/bin/dahomey-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Dahomey Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Dahomey Core version 0.18.0.
...
- Testing or debugging of implementations of the Dahomey P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Dahomey Core. Dahomey Core logs debug messages
...
- OpenSSL is no longer used by Dahomey Core.  (#17265)

- BIP70 support has been fully removed from Dahomey Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/dahomey/dahomey/issues/16599),
  [PR #16702](https://github.com/dahomey/dahomey/pull/16702), and the
  `dahomeyd help` for more information.  This option is experimental and
...
- All custom settings configured when Dahomey Core starts are now
...
- The "Start Dahomey Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://dahomeycore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Dahomey Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Dahomey Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/dahomey-core/HWI) for
  signing.  Future versions of Dahomey Core should support a GUI option
...
- Dahomey Core's automatically-generated source code documentation is
  now available at https://doxygen.dahomeycore.org.  (#17596)
...
- The `dahomey-cli` utility used with the `-getinfo` parameter now
...
- The `dahomey-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `dahomeystrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_dahomey-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_dahomey` binary (hebasto)
...
- #18702 Fix ASLR for dahomey-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate dahomey schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest dahomey.conf (sdaftuar)
...
- #18022 Fix appveyor `test_dahomey` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (dahomey-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone dahomey-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Dahomey Core files (hebasto)
...
- #17119 Fix broken dahomey-cli examples (andrewtoth)
...
- #17503 Remove bitness from dahomey-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.dahomeycore.org (laanwj)
...
- #17648 Rename wallet-tool references to dahomey-wallet (hel-o)
...
- #18341 Replace remaining literal DEY with `CURRENCY_UNIT` (domob1812)
...
- #16115 On dahomeyd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from dahomey-cli -getinfo (malevolent)
