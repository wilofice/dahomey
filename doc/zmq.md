...
The Dahomey Core daemon can be configured to act as a trusted "border
router", implementing the dahomey wire protocol and relay, making
...
The ZeroMQ feature in Dahomey Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building dahomeyd:
...
terminator). These options can also be provided in dahomey.conf.
...
From the perspective of dahomeyd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into dahomeyd directly. Furthermore, no information is
...
using. Dahomeyd appends an up-counting sequence number to each
